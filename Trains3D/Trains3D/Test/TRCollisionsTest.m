...
- (void)testStraight {
    [intTo(1, 100) forEach:^void(id i) {
        [CNLog applyText:[NSString stringWithFormat:@"-- %@ ------", i]];
...
- (void)doTest1ForLevel:(TRLevel*)level form:(TRRailForm*)form {
    TRTrainActor* t1 = [TRTrainActor trainActorWith_train:[TRTrain trainWithLevel:level trainType:TRTrainType.simple color:TRCityColor.green __cars:^id<CNSeq>(TRTrain* _) {
    ...
    TRTrainActor* t2 = [TRTrainActor trainActorWith_train:[TRTrain trainWithLevel:level trainType:TRTrainType.simple color:TRCityColor.orange __cars:^id<CNSeq>(TRTrain* _) {
        return (@[[TRCar carWithTrain:_ carType:TRCarType.engine], [TRCar carWithTrain:_ carType:TRCarType.engine]]);
    } speed:0]].actor;
...
- (void)testCross {
    ...
    [level testRunTrain:[TRTrainActor trainActorWith_train:t1] fromPoint:p1];
    ...
    [level testRunTrain:[TRTrainActor trainActorWith_train:t2] fromPoint:p2];
    id<CNSet> cols = [self checkLevel:level];
    [self assertTrueValue:[cols isEmpty]];
