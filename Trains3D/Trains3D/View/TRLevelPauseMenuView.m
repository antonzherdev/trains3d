...
- (void)draw {
    ...
    egBlendFunctionApplyDraw(egBlendFunctionStandard(), ^void() {
        ...
        [_resumeLine draw];
        [_font drawText:[TRStr.Loc resumeGame] color:GEVec4Make(0.0, 0.0, 0.0, 1.0) at:GEVec2Make(p.x + 35, p.y + 112) alignment:egTextAlignmentBaselineX(-1.0)];
        _restartLine.p0 = GEVec2Make(p.x, p.y + 50);
        _restartLine.p1 = GEVec2Make(p.x + _width, p.y + 50);
        [_restartLine draw];
        [_font drawText:[TRStr.Loc restartLevel] color:GEVec4Make(0.0, 0.0, 0.0, 1.0) at:GEVec2Make(p.x + 35, p.y + 62) alignment:egTextAlignmentBaselineX(-1.0)];
