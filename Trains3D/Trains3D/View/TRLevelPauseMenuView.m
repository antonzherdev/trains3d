...
- (void)draw {
    ...
    egBlendFunctionApplyDraw(egBlendFunctionStandard(), ^void() {
        ...
        [_resumeLine draw];
        [_font drawText:[TRStr.Loc resumeGame] color:GEVec4Make(0.0, 0.0, 0.0, 1.0) at:GEVec3Make(p.x + 35, p.y + 112, 0.0) alignment:egTextAlignmentBaselineX(-1.0)];
        _restartLine.p0 = GEVec2Make(p.x, p.y + 50);
        _restartLine.p1 = GEVec2Make(p.x + _width, p.y + 50);
        [_restartLine draw];
        [_font drawText:[TRStr.Loc restartLevel] color:GEVec4Make(0.0, 0.0, 0.0, 1.0) at:GEVec3Make(p.x + 35, p.y + 62, 0.0) alignment:egTextAlignmentBaselineX(-1.0)];
        _mainMenuLine.p0 = GEVec2Make(p.x, p.y);
        _mainMenuLine.p1 = GEVec2Make(p.x + _width, p.y);
        [_mainMenuLine draw];
        [_font drawText:[TRStr.Loc mainMenu] color:GEVec4Make(0.0, 0.0, 0.0, 1.0) at:GEVec3Make(p.x + 35, p.y + 12, 0.0) alignment:egTextAlignmentBaselineX(-1.0)];
        glLineWidth(1.0);
