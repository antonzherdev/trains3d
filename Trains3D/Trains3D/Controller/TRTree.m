...
- (void)_init {
    __trees = [[[[intRange(((NSInteger)(_rules.thickness * [_map.allTiles count] * 1.1))) chain] mapF:^TRTree*(id _) {
        ...
        return [TRTree treeWithTreeType:((TRTreeTypeR)(nonnil([[TRForestType_Values[_rules.forestType].treeTypes chain] randomItem]))) position:geVec2AddVec2(pos, geVec2ApplyVec2i(tile)) size:GEVec2Make((((float)(cnFloatRndMinMax(0.9, 1.1)))), (((float)(cnFloatRndMinMax(0.9, 1.1)))))];
    }] sort] toArray];
    __treesCount = [__trees count];
}
