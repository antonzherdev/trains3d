    ...
    describe(@"Square isometric(Sso)", ^{
        describe(@"should determine full and partial tiles. ", ^{
            describe(@"For map size 2x3", ^{
                EGMapSso* m = [EGMapSso mapSsoWithSize:EGISizeMake(2, 3)];
                it(@"the tile 0:2 should be full", ^{
                    [[theValue([m isFullTile:EGIPointMake(0, 2)]) should] beTrue];
                });
                it(@"the tile 1:0 should be full", ^{
                    [[theValue([m isFullTile:EGIPointMake(1, 0)]) should] beTrue];
                });
                it(@"the tile -1:1 should be full", ^{
                    [[theValue([m isFullTile:EGIPointMake(-1, 1)]) should] beTrue];
                });
                it(@"the tile -1:0 should not be full but should be partial", ^{
                    [[theValue([m isFullTile:EGIPointMake(-1, 0)]) should] beFalse];
                    [[theValue([m isPartialTile:EGIPointMake(-1, 0)]) should] beTrue];
                });
                it(@"the tile -2:1 should not be full but should be partial", ^{
                    [[theValue([m isFullTile:EGIPointMake(-2, 1)]) should] beFalse];
                    [[theValue([m isPartialTile:EGIPointMake(-2, 1)]) should] beTrue];
                });
                it(@"the tile -3:1 should not be full and should not be partial", ^{
                    [[theValue([m isFullTile:EGIPointMake(-3, 1)]) should] beFalse];
                    [[theValue([m isPartialTile:EGIPointMake(-3, 1)]) should] beFalse];
                });
